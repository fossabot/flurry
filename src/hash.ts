...
// Minimal sync SHA-256 implementation for environments without Web Crypto
...
function sha256Pure(data: Uint8Array): Uint8Array {
  ...
  const zeroCount = (55 - (data.length % 64) + 64) % 64;
  const padded = new Uint8Array(data.length + 1 + zeroCount + 8);
  padded.set(data);
  padded[data.length] = 0x80;
  // Write bit length as big-endian 64-bit (only lower 32 bits for inputs < 512MB)
